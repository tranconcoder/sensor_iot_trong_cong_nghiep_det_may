...
import { convertObjectConfigToString, handleCodecData, handleEnd, handleError, handleProgress, handleStart, } from "../utils/ffmpeg.util";
import { DRAWTEXT_COLOR, DRAWTEXT_FONTPATH, FONTSIZE, FRAMESIZE, FRAMESIZE_WIDTH, FRAME_PADDING_X, FRAME_PADDING_Y, } from "../config/ffmpeg.config";
...
const videoFilterConfig2 = Object.assign(Object.assign({}, videoFilterConfig), { text: "SecurityCam", get x() {
        return parseInt(FRAMESIZE_WIDTH - FONTSIZE * (19 / 30) * this.text.length + "");
    } });
export const ffmpegCommand = Ffmpeg({ priority: 0 })
    ...
    `-vf ` +
        `hflip,` +
        `drawtext=${convertObjectConfigToString(videoFilterConfig, "=", ":")},` +
        `drawtext=${convertObjectConfigToString(videoFilterConfig2, "=", ":")}`,
    "-b:v 2M",
    "-fps_mode auto",
    "-pix_fmt yuv420p",
